<template>
	<view>
		...
		<view class="promotion">
			<view class="text">拼团专区</view>
			<view class="list" v-if="groupHotGoodsList && groupHotGoodsList.length>0">
				<view
					class="column"
					v-for="(row, index) in groupHotGoodsList"
					@tap="toPromotion(row)"
					:key="index"
				>
					<view class="top">
						<view class="title">{{row.goods.title}}</view>
					...
					<view class="right"><image :src="row.goods.pic"></image></view>
				</view>
			</view>
		</view>
		<!-- 商品列表 -->
			...
			<view class="product-list">
				...
				>
					...
					<view class="info">
						...
						<view class="slogan">{{ product.sale }}</view>
					</view>
				</view>
			</view>
...
export default {
	...
	this.pageNum = this.pageNum+1 ;
		let params = {'pageNum':this.pageNum};
                			  let goodsList =  await Api.apiCall('get',Api.goods.goodsList,params);
        		let len = this.productList.total;


        	  this.productList = this.productList.concat(goodsList.records);
        	  if (this.productList.length >= len) {
...
let params = {'pageNum':this.pageNum};
	...
	methods: {
		...
		async loadPromotion() {
			...
			//检查倒计时
			for (let i = 0; i < groupHotGoodsList.length; i++) {
				let row = groupHotGoodsList[i];
				if (row.countdown) {
					let h = '00',
						m = '00',
						s = '00';
					...
					groupHotGoodsList[i].countdown = { h: h, m: m, s: s };
				}
			}
